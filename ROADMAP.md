...
- [ ] **3.1** Redis 滑动窗口限流（默认 100/min，LLM 端点 20/min）实现 + 测试（G4）
- [ ] **3.2** RBAC 边界回归测试（401/403 覆盖每个受保护端点，默认拒绝未声明权限端点）
- [ ] **3.3** CI 增加 secret 扫描（gitleaks/trufflehog）（G3）
- [ ] **3.4** 生产 CORS 校验（禁 `*` + credentials）；LLM API Key 仅服务端持有审计
- [ ] **3.5** 文件上传白名单 + 50MB 限制回归测试

**验收**: 401/403 边界全绿；secret 扫描无泄漏；限流中间件可观测。
...
- [ ] **4.1** K8s 补 HPA + PDB，frontend 升至 2 replicas，ConfigMap/Secret 分离（G2）
...
**验收**: K8s 一键部署 2+ replicas 健康；指标/告警可查。
